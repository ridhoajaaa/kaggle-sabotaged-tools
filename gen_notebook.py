...
def main() -> None:
    ...
    cells.append(
        code_cell(
            ...
            "print('Model tersedia:', list(kbench.llms))\n"
            "try:\n"
            "    LLM = kbench.llms['google/gemini-2.5-flash']  # ganti sesuai daftar\n"
            "except Exception:\n"
            "    LLM = kbench.llm\n\n"
            "# Retry otomatis untuk 429 'heavy load' yang sifatnya sementara:\n"
            "import time\n"
            "def run_retry(task, llm, attempts=4, delay=60):\n"
            "    for i in range(attempts):\n"
            "        try:\n"
            "            return task.run(llm)\n"
            "        except Exception as exc:\n"
            "            msg = f'{type(exc).__name__}: {exc}'\n"
            "            if '429' in msg or 'rate_limit' in msg.lower() or 'heavy load' in msg.lower():\n"
            "                print(f'[{i+1}/{attempts}] 429 heavy load — tunggu {delay}s...')\n"
            "                time.sleep(delay); continue\n"
            "            raise\n"
            "    raise RuntimeError('Masih rate-limited — tunggu beberapa menit / ganti model.')\n\n"
            "# Run utama: dunia teracaukan (PENTING: run() butuh argumen llm):\n"
            "run_retry(sabotaged_tools_task, LLM)\n"
            "print_breakdown()\n\n"
